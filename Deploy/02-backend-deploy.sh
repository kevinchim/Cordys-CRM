...
# 复制 JAR 到部署目录，并解压类文件覆盖旧版本
echo "      复制 JAR 并提取类文件..."
docker cp $COMPILE_CONTAINER:/tmp/CordysCRM-1.7.0/backend/crm/target/crm-main.jar $DEPLOY_DIR/crm-main.jar
docker cp $COMPILE_CONTAINER:/tmp/CordysCRM-1.7.0/backend/framework/target/framework-main.jar $DEPLOY_DIR/framework-main.jar

# 解压类文件到 overlay，启动时挂载覆盖旧类
