<template>
  <n-scrollbar content-class="overflow-auto">
    <div class="welltrans-push-container p-[24px]">
      <!-- 配置区域 -->
      <CrmCard :title="t('menu.settings.welltransPush')" auto-height hide-footer class="mb-[16px]">
        <n-form
          ref="formRef"
        ...
        >
          ...
          <n-form-item :label="t('system.welltrans.autoPush')" path="autoPushEnabled">
            <n-switch v-model:value="form.autoPushEnabled" :rubber-band="false" />
          </n-form-item>
          <n-form-item :label="t('system.welltrans.manualPush')" path="manualPushEnabled">
            <n-switch v-model:value="form.manualPushEnabled" :rubber-band="false" />
          </n-form-item>
        </n-form>
        <n-divider class="!m-0" />
        <div class="my-[12px] mr-[24px] flex justify-end gap-[8px]">
          ...
          >
            {{ pushing ? t('system.welltrans.pushing') : t('system.welltrans.executePush') }}
          </n-button>
          <n-button v-permission="['SYSTEM_SETTING:UPDATE']" type="primary" :loading="saving" @click="handleSaveConfig">
      ...
      <!-- 推送结果 -->
      <CrmCard v-if="lastResult" :title="t('system.welltrans.pushResult')" auto-height hide-footer class="mb-[16px]">
        <n-descriptions :column="4" label-placement="left" class="p-[16px]">
          <n-descriptions-item :label="t('system.welltrans.totalCount')">{{ lastResult.totalCount }}</n-descriptions-item>
          <n-descriptions-item :label="t('system.welltrans.successCount')">
            <span class="text-[var(--success-green)]">{{ lastResult.successCount }}</span>
          </n-descriptions-item>
          <n-descriptions-item :label="t('system.welltrans.failCount')">
            <span :class="lastResult.failCount > 0 ? 'text-[var(--error-red)]' : ''">{{ lastResult.failCount }}</span>
          </n-descriptions-item>
          <n-descriptions-item :label="t('system.welltrans.errorMessage')">
            <span v-if="lastResult.errorMessage" class="text-[var(--error-red)]">{{ lastResult.errorMessage }}</span>
            <span v-else>-</span>
          </n-descriptions-item>
        </n-descriptions>
      </CrmCard>
      ...
      <!-- 推送历史 -->
      <CrmCard :title="t('system.welltrans.pushHistory')" auto-height hide-footer>
        <n-data-table
          :columns="logColumns"
          :data="pushLogs"
          :loading="logsLoading"
...
<script setup lang="ts">
  ...
  import type { WelltransPushLog, WelltransPushResult } from '@lib/shared/models/system/business';
  ...
  const logColumns: DataTableColumn<WelltransPushLog>[] = [
    {
      ...
      width: 100,
      render: (row) =>
        row.triggerType === 'AUTO' ? t('system.welltrans.triggerAuto') : t('system.welltrans.triggerManual'),
    },
    {
