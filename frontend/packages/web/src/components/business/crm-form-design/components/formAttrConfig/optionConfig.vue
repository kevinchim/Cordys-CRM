<template>
  ...
  <component
    :is="getComponent"
    v-else-if="
      fieldConfig.optionSource === 'custom' && fieldConfig.customOptions
    "
    :value="fieldConfig.defaultValue"
    :disabled="props.disabled"
  >
    ...
    >
      ...
      >
        ...
        <n-input
          ...
          :disabled="props.disabled"
          :status="
            fieldConfig.customOptions.some(
              (e) => e.value !== item.value && e.label === item.label
            )
              ? 'error'
              : undefined
          "
        ...
        >
          <template #trigger>
            <n-button
              ...
              :disabled="
                props.disabled ||
                fieldConfig.customOptions?.length === 1 ||
                fieldConfig.defaultValue === item.value
              "
  ...
  <div
    v-if="fieldConfig.optionSource === 'custom'"
  ...
  >
    ...
    <div
      :class="
        props.disabled ||
        fieldConfig.customOptions?.some((item) => item.value === 'other')
...
async function loadDictCategories() {
  try {
    ...
    dictCategoryOptions.value = (cats || []).map((c: any) => ({
      label: `${c.name} (${c.code})`,
      value: c.code,
    }));
  } catch {
...
async function handleDictCodeChange(code: string) {
  ...
  try {
    ...
    fieldConfig.value.options = (items || []).map((item: any) => ({
      label: item.label,
      value: item.value,
    }));
...
const getRefData = async (key: string) => {
  ...
  return res.fields
    .filter(
      (field) =>
        field.optionSource === "custom" && field.type === fieldConfig.value.type
    )
    .map((field) => ({
      label: field.name,
      value: field.id,
...
watch(
  ...
  async () => {
    ...
    if (
      fieldConfig.value.optionSource === "ref" &&
      fieldConfig.value.refFormKey
    ) {
...
function handleAddOption() {
  ...
  if (!fieldConfig.value.customOptions?.some((e) => e.value === "other")) {
    fieldConfig.value.customOptions?.push({
      label: t("crmFormDesign.option", {
        i: fieldConfig.value.customOptions.length + 1,
      }),
      value: getGenerateId(),
    });
  } else {
    fieldConfig.value.customOptions?.splice(
      fieldConfig.value.customOptions.length - 1,
      0,
      {
        label: t("crmFormDesign.option", {
          i: fieldConfig.value.customOptions.length,
        }),
        value: getGenerateId(),
      }
    );
...
function handleAddOtherOption() {
  if (
    props.disabled ||
    fieldConfig.value.customOptions?.some((e) => e.value === "other")
  ) {
    return;
  }
  fieldConfig.value.customOptions?.push({
...
function setDefaultValue() {
  if (isMultiple.value) {
    fieldConfig.value.defaultValue = fieldConfig.value.defaultValue?.filter(
      (e: any) =>
        fieldConfig.value.customOptions?.some((item) => item.value === e)
    );
  } else if (
    fieldConfig.value.customOptions?.every(
      (e) => e.value !== fieldConfig.value.defaultValue
    )
  ) {
    fieldConfig.value.defaultValue = "";
  }
...
function handleShowBatchEditModal() {
  ...
  batchEditValue.value =
    fieldConfig.value.customOptions?.map((e) => e.label).join("\n") || "";
}
  ...
  if (resArr.length === 0) {
    ...
    const newOptions = resArr
      ...
      .map((e) => ({
        label: e.slice(0, 50),
        value:
          fieldConfig.value.customOptions?.find((item) => item.label === e)
            ?.value || getGenerateId(),
      }));
...
<style lang="less">
// 没数据的样式
.form-design-cascader
  .n-cascader-option.n-cascader-option--disabled:has(.no-data) {
